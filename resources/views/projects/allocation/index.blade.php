      ...
      <table   class="table table-bordered   search-table">
        ...
        <tbody id="storeGroup">
          ...
          @foreach($employees as $employee)
              ...
              <td><input @foreach($allocatedEmployee as $list) @if($list->employee_id==$employee->id) checked disabled  @endif   @endforeach type="checkbox" name="employee_detail"  value="{{$employee->id}}" class="employee-data"></td>
              </tr>
                @endforeach
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
          ...
          <tbody>
            ...
            <tr>
                {!! Form::model($project, ['route' => ['allocationUpdate', $project->id], 'method' => 'post' ]) !!} 
                    <td>{{$list->first_name}}<p></p>{{$list->last_name}}</td>
                    {{csrf_field()}} 
                    <input type="hidden" name="mapping_id" value="{{$list->id}}">
                    <input type="hidden" name="emp_id" value="{{$list->employee_id}}">
                    <td>
                      <select class="form-control" required=""   class="form-control" name="role">
                        <option value="">--Select Roles--</option>
                        @foreach($roles as $role)
                        <option @if($list->role_id==$role->id) selected  @endif value="{{$role->id}}">{{$role->designation}}</option>
                        @endforeach
                    </select>
                </td>
        ...
        <td style="text-align: center;">
            ...
            <button  type="submit" style="background: white;border:none;outline: none; width: 70px;">
              
                <i class="fa fa-pencil-square-o" aria-hidden="true" style="font-size: 27px;"></i>
             </button>
             {!! Form::close() !!}
        </td> 
        ...
        {!! Form::model($project, ['route' => ['allocation.change_status', 'id' => $project->id], 'method' => 'post' ]) !!} 
                 ...
                 <button type="submit" class="status" onclick="return confirm('Do you want to deactivate?');"><img src="{{URL::asset('public/images')}}/activate.png" style=""></button>
          ...
          {!! Form::model($project, ['route' => ['allocation.change_status', 'id' => $project->id], 'method' => 'post' ]) !!} 
               ...
               <input type="hidden" name="actt" value="1">
                 ...
                 <button type="submit" class="status" onclick="return confirm('Do you want to activate?');"><img src="{{URL::asset('public/images')}}/deactivate.png" style=""></button>
