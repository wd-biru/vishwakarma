...
<div class="row" style="background: white;margin:1px">
   <div class="col-md-12">
        <form action="{{route('indentResorurce.downloadPDF',$project->id)}}"  method="post">
           ...
           @endforeach
           @endif
          <button class="btn btn-primary" style="float: right;margin-bottom:2%; margin-top:2%!important;">Download PDF</button>
       </form>
    ...
    if (isset($vendor_mapping_detail)) {
        $indentdata = IndentMaster::where('indent_id', $vendor_mapping_detail->indent_id)->first();
    }
    ...
    if ($indentdata->current_status==null) {
        ...
        $workflow = App\Http\Controllers\Controller::getPoWorkFlow($purchaseData, 'Po Flow');
        $stage = $purchaseData->toCheckStage($workflow, $purchaseData);
    }
    else
        {
            $workflow = App\Http\Controllers\Controller::getWorkFlow($vendor_mapping_detail, 'Indent Flow');
...
//    if (($workflowId->place_from_id == $workflowId->place_to_id)&&($indentdata->current_status==$workflowPlace->place_name)) {
//
//            $workflow = App\Http\Controllers\Controller::getPoWorkFlow($purchaseData, 'Po Flow');
//            $stage = $vendor_mapping_detail->toCheckStage($workflow, $purchaseData);
//
//    } else {
...
//        $stage = $vendor_mapping_detail->toCheckStage($workflow,$purchaseData);
//dd($stage,$vendor_mapping_detail->is_active);
...
@if(isset($vendor_mapping_detail))
  <div class="col-md-6">
    ...
    <div class="panel-heading">
        <h3 class="panel-title"><sapn>Indent Details</sapn><span class="pull-right"> Status : <?php echo ($stage==null)? "Done" : $stage ; ?> </span></h3>
         ...
         <div class="panel panel-info">
              <div class="panel-heading">
                  <div class="row">
                      <div class="col-md-12" style="font-size: 15px"> 
                        <p><span> Indent Number: {{$vendor_mapping_detail->indent_id}}</span> <span style="float: right;">Created Date: {{date('d-m-Y', strtotime($vendor_mapping_detail->created_at))}}</span></p>
                        <p>
          ...
          <div class="panel panel-info">
              <div class="panel-heading">
                  <div class="row">
                    <div class="col-md-12">
                    <h3 style="font-size: 20px; margin-top: 0px; margin-bottom: 8px;">Indent Track:</h3>
                  </div>
                      <div class="col-md-12" style="font-size: 15px;">
                          ...
                          <div class="col-md-6">
                             ...
                             <span>{{$employee->first_name}}{{$employee->last_name}}({{$employee->getDesignation->designation}})
                                @elseif($users->user_type == "portal")
                                   <span>{{$users->name}}{{$users->surname}}(Admin)</span>
                                @endif
                              @endif
                         </div>
                         <div class="col-md-3" >
                  ...
                  <div class="row">
                      @if(Auth::user()->user_type != "employee")
                          @if ($workflowId->place_from_id != $workflowId->place_to_id)
                              <div class="col-md-12">
                                  <h3 style="font-size: 20px; margin-top: 0px; margin-bottom: 8px;">Share With:</h3>
                              </div>
                          @endif
                      @else
                          <div class="col-md-12">
                              <h3 style="font-size: 20px; margin-top: 0px; margin-bottom: 8px;">Approval And
                                  Rejection:</h3>
                          </div>
                      @endif
                      <div class="col-md-12" style="font-size: 15px">
                                  ...
                                  @if($userStage == $stage || Auth::user()->user_type != "employee")
                                      ...
                                      @if ($indentdata->current_status==null)
                                          ...
                                          @if($vendor_mapping_detail->getVendorsPrice()->count()>0)
                                              ...
                                              <button class="btn btn-primary form-control">Link To View Vendor Quote
                                              </button>
                                              {!! Form::close() !!}
                                          @else
...
{{--{{dd($userStage,$stage)}}--}}
                                  {{--{{dd($userStage == $stage,Auth::user()->user_type == "portal",$vendor_mapping_detail->is_active==1,$stage!=null,$stage--}}
                            {{--,$indentdata->current_status,$workflowId->place_name)}}--}}
                                  {{--{{dd($workflow,$stage)}}--}}
                                  @if(($userStage == $stage || Auth::user()->user_type == "portal") && $vendor_mapping_detail->is_active==1 &&  $stage!=null)
                                      @if ($indentdata->current_status!=null)
                                          <form action="{{route('indent.workflow.change',[$project->id,$vendor_mapping_detail->id])}}"
                                                method="post">
                                              {{csrf_field()}}
                                              ...
                                              <button style="float: right;margin-bottom:2%; margin-top:2%!important;"
                                                      ...
                                                      class="btn btn-success">{{$stage}}</button>
                                              <button style="float: left;margin-bottom:2%; margin-top:2%!important;"
                                                      name="button" type="submit" value="Rejection"
                              ...
                              @elseif($stage!=null && $vendor_mapping_detail->is_active==0)
                                  @if(Auth::user()->user_type == 'employee')
                                      ...
                                      ?>
                                      @if($userStage!="")
                                          <h5 style="color:red;">Note* : Indent Is Rejected So plese Re-create or Edit
                                              Indent</h5>
...
<div class="col-md-6">
      <div class="table-responsive" style="width:100%">
                <table id="data-table" class="table table-striped table-hover table-condensed table-bordered search-table">
                    @if(count($indent)> 0)
