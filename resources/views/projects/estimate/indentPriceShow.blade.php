...
@section('content')
                                    ...
                                    @foreach($indent as $list)
                                        ...
                                        <tr class="tab-text-align">
                                            ...
                                            <td><input type="checkbox"
                                                       @if(!empty($chkPurchaseOrder)) @foreach($chkPurchaseOrder as $val) @if($val->item_id==$list->item_id) checked
                                                       disabled @endif   @endforeach  @endif type="checkbox"
                                                       name="item_id" data-project_id="{{$list->project_id}}"
                                            ...
                                            @foreach($vendor_indent_mapping  as  $vendor_price)
                                                ...
                                                if ($priceVendor->price == null) {

                                                    $price = ($priceVendor == false) ? "" : $priceVendor->per_day_price;
                                                    $price_recovery = ($priceVendor == false) ? "" : $priceVendor->recovery_price;
                                                ...
                                                ?>
                                                    @if( ($priceVendor->price == null))
                                                        <td>{{$price}} &nbsp; {{$price_recovery}}<b
                                                                    style="color:{{$LowColor}}">{{$lowest}}</b> {{$remarks}}
                                                        </td>
                                                    @else
                                                        <td>{{$price}}<b
                                                                    style="color:{{$LowColor}}">{{$lowest}}</b> {{$remarks}}
    ...
    <div class="row purchase_order">
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">&nbsp;Purchase Order No :ORD-{{$purchase_order_number}}
                        -{{ now()->year }}</h3>
                </div>
                <div class="panel-body">
                    <div class="col-md-12">
                        ...
                        <input type="hidden" class="form-control"
                               value="ORD-{{$purchase_order_number}}-{{ now()->year }}" name="purchase_order_no"
                        ...
                        <div class="col-md-6">
                                ...
                                <div class="col-md-12">
                                    <fieldset>
                                        <input type="text" class="form-control datepicker" name="from_date"
                                               id="to_from_date" autocomplete="off" required>
                                    </fieldset>
                                </div>
                            ...
                            <br>
                            <div class="form-group">
                                <label class="control-label col-md-4">Voucher No&nbsp;<span
                                            style="color: #f70b0b">*</span></label>
                                <div class="col-md-12">

                                    <input type="number" class="form-control" name="voucher_no" id="voucher_no"
                                           required>
                                </div>
                            </div>
