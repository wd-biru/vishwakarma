...
@section('content-project')
    ...
    <div class="col-md-12 card" style="padding:15px">
        <div class="col-md-2 date_change">
            ...
            <input type="text" class="datetimepicker form-control" autocomplete="off"
                   @if($from_date_input!=0) value="{{$from_date_input}}" @endif  id="from" required>
        </div>
        <div class="col-md-2 date_change">
            <label>To Date</label>
            <input type="text" class="datetimepicker form-control" autocomplete="off"
                   @if($to_date_input!=0) value="{{$to_date_input}}" @endif  id="to" required>
        </div>
        <div class="col-md-1">
            <label>&nbsp;</label>
            <br>
                            ...
                            for (var i = 0; i < result.data.length; i++) {
                                if (result.data[i].qty != 0) {
                                    if (result.data[i].sumqty == null) {
                                        godown_items += ' <tr id="' + result.data[i].id + '"><td>' + result.data[i].invoice_no + '</td><td>' + new Date(result.data[i].invoice_date).toString('dd-MM-yyyy') + '</td><td>' + new Date(result.data[i].inward_date).toString('dd-MM-yyyy') + '</td><td>' + result.data[i].material_name + '</td><td style="text-align: left;">' + result.data[i].qty + '</td>'+

                                        // Start code is commented by the prem
                                        '<!--<td  style="text-align: center;"><div class="btn-group"><button type="button" class="btn btn-danger dropdown-toggle" data-toggle="dropdown"> <span class="caret"></span> <span class="sr-only">Toggle Dropdown</span> Action</button><ul class="dropdown-menu" role="menu"><li><a onclick="appenddata(' + result.data[i].id + ',' + result.data[i].qty + ');" >Edit</a></li><li><a id="' + result.data[i].id + '" data-itemqty="" class="class-btn-delete-list">Delete</a></li></ul></div></td>-->'+
                                         // End code is commented by the prem
                                        ...
                                        godown_items += ' <tr id="' + result.data[i].id + '"><td>' + result.data[i].invoice_no + '</td><td>' + new Date(result.data[i].invoice_date).toString('dd-MM-yyyy') + '</td><td>' + new Date(result.data[i].inward_date).toString('dd-MM-yyyy') + '</td><td>' + result.data[i].material_name + '</td><td style="text-align: left;">' + result.data[i].qty + '</td>'+
                                          ...
                                          // Start code is commented by the prem
                                        '<!--<td  style="text-align: center;"><div class="btn-group"><button type="button" class="btn btn-danger dropdown-toggle" data-toggle="dropdown"> <span class="caret"></span> <span class="sr-only">Toggle Dropdown</span> Action</button><ul class="dropdown-menu" role="menu"></ul></div></td>-->'+
                                          // End code is commented by the prem
                                        '</tr>';
