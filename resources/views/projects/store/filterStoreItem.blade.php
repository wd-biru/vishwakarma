            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="card col-md-12 ">
                        ...
                        <label style="margin-left: 21%">Material Group Items</label>&nbsp;&nbsp;&nbsp;
                        <select name="group_id" class="form-control" id="getMaterialItemList"
                                style="padding-bottom: 9px;margin-bottom: 18px;">
                            <option value="">Select Material Group Item</option>
        ...
        @section('script')
            ...
            <script>
                $(document).ready(function () {
                    $('#getMaterialItemList').on('change', function () {
                        var group_id = $('#getMaterialList option:selected').val();
                        var item_id = $(this).val();
                        var group_type_id = $('#get_materialGroupBy_itemType option:selected').val();

                        $.ajax({
                            url: "{{route('masterStore.getFilterItemList')}}",
                            type: 'get',
                            data: {
                                "group_id": group_id,
                            ...
                            success: function (data) {
                                $("#itemListGroupItem").DataTable().destroy();
                                $('#getListItem').html(data);
                                $("#itemListGroupItem").DataTable({
                                    "aLengthMenu": [100]
                                });
                            }
                        });
                    ...
                    $('#getMaterialList').on('change', function () {
                        ...
                        $.ajax({
                            url: "{{route('getMaterialGroupItem')}}",
                            type: 'get',
                            data: {
                                "group_type_id": group_type_id,
                                "group_id":group_id,
                            ...
                            success: function (data) {
                                ...
                                $("#getMaterialItemList").empty();
                                $("#getMaterialItemList").append('<option name="">Select Material Group Item</option>');
                                $.each(data, function (key, value) {
                                    $("#getMaterialItemList").append('<option value="' + key + '">' + value + '</option>');
                                });
                            }
