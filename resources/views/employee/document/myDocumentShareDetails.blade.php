...
<div class="row">
	...
	<div class="col-md-12">
		<div class="panel panel-default">
			<div class="panel-heading"><h3 class="panel-title">Document Share Details Here </h3></div>
							...
							<table class="table search-table">
								...
								<tbody>
									...
									<tr>
										...
										<td>{{$data->upload_by}}</td>
										@if(($data->for_approval)==NULL)
										    <td>-------</td>
										@elseif(($data->for_approval)==0)
										   <td>ADMIN</td>
										@else
											<td>{{$data->getApprovalName->first_name}}  {{$data->getApprovalName->last_name}}</td>
										@endif
										@if(($data->status=='APPROVED')&&($data->for_approval)!=NULL)
										<td style="color: green;">{{$data->status}}</td>
										@elseif(($data->status=='REJECTED')&&($data->for_approval)!=NULL)
										<td style="color: red;">{{$data->status}}</td>
										@elseif(($data->status=='PENDING')&&($data->for_approval)!=NULL)
										<td style="color: orange;">{{$data->status}}</td>
										@elseif(($data->status=='PENDING')&&($data->for_approval)==NULL)
										<td style="color: blue;">SHARED</td>
										@endif	
